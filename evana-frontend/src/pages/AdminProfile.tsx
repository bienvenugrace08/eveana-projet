...
const AdminProfile: React.FC = () => {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCurrentPassword('');
      setNewPassword('');
      setSuccess('Mot de passe administrateur changé avec succès !');
      setTimeout(() => setSuccess(null), 3000);
    } catch (err: any) {
  ...
  return (
    <div className="py-12 md:py-16 container mx-auto px-4 lg:px-6 max-w-4xl space-y-8 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-200 dark:border-slate-700">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">Profil Administrateur</h1>
          <p className="text-xs text-slate-400">Gérez vos privilèges et accès d'administration.</p>
        </div>
        <span className="self-start sm:self-auto inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-100 dark:bg-red-950/40 text-danger-dark text-xs font-semibold">
          <Shield className="w-3.5 h-3.5 text-danger" /> Rôle Administrateur
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 flex flex-col items-center text-center space-y-4 shadow-soft">
          <div className="w-24 h-24 rounded-full bg-gradient-to-br from-red-500 to-primary flex items-center justify-center text-white text-3xl font-bold shadow-soft">
        ...
        <div className="md:col-span-2 space-y-6">
          <form onSubmit={handleSaveInfo} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 space-y-4 shadow-soft">
            <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <User className="w-4 h-4 text-primary" /> Détails du profil
            </h3>
          ...
          <form onSubmit={handleChangePassword} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 space-y-4 shadow-soft">
            <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <Key className="w-4 h-4 text-primary" /> Sécurité administrateur
            </h3>
